...
export default function QRGeneratorPage() {
  ...
  const filteredSepeda = searchNomorSeri
    ? sepedaList.filter((s) => s.nomorSeri.toLowerCase().includes(searchNomorSeri.toLowerCase()))
    : []
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Ketik nomor seri sepeda untuk membuat QR Code</CardDescription>
            </CardHeader>
            <CardContent>
              <Input
              ...
              />
              {searchNomorSeri && (
                <div className="max-h-40 overflow-auto border rounded bg-white shadow">
                  {filteredSepeda.length === 0 ? (
                    ...
                    filteredSepeda.map((sepeda) => (
                      <div
                        ...
                        className={`p-2 cursor-pointer hover:bg-gray-100 ${selectedNomorSeri === sepeda.nomorSeri ? "bg-gray-200" : ""}`}
              ...
              {selectedNomorSeri && (
                <div className="mt-2 text-sm text-green-600">
                  Nomor Seri terpilih: <span className="font-bold">{selectedNomorSeri}</span>
                </div>
              )}
            </CardContent>
          </Card>
