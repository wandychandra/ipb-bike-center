...
export default function ReportDashboard() {
    ...
    const fetchData = async () => {
      ...
      setPeminjamanData(
        (data || []).map((item: any) => ({
          ...
          DataSepeda: Array.isArray(item.DataSepeda)
            ? item.DataSepeda[0]
            : item.DataSepeda,
          StatusPeminjaman: Array.isArray(item.StatusPeminjaman)
            ? item.StatusPeminjaman[0]
  ...
  useEffect(() => {
    const filtered = peminjamanData.filter((item) => {
      ...
      return (
        peminjamanDate.getMonth() + 1 === selectedMonth &&
        peminjamanDate.getFullYear() === selectedYear
      );
    });
    ...
    <div className='space-y-6'>
      {isLoading ? (
        <div className='space-y-4'>
          <Skeleton className='h-[120px] w-full sm:h-[200px]' />
              ...
              <ReportTable
                data={filteredData.map((item) => ({
                  ...
                  DataSepeda: {
                    ...item.DataSepeda,
                    nomorSeri:
                      item.DataSepeda?.nomorSeri ?? item.nomorSeriSepeda ?? ''
                  }
                }))}
              />
            </TabsContent>
